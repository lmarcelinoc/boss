...
class SwaggerOnlyAppModule {}

async function bootstrap() {
  const app = await NestFactory.create(SwaggerOnlyAppModule, {
...
**Note**: This documentation represents the complete authentication system. Some endpoints may require specific environment setup or feature flags to be enabled.
    `)
    .setVersion('1.0')
    .addBearerAuth()
  ...
  console.log(`🚀 Swagger-Only API server running on port ${port}`);
...
bootstrap().catch(err => {
  console.error('Failed to start Swagger-only server:', err);
  process.exit(1);
});
